        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            else if (context.Request.Path.StartsWithSegments("/API"))
            {
               ...
               /* if (context.Request.Query.Keys.Contains("id"))
                ...
                {
                    // /API/<context>/chargePointId?idTag=&&connectorId
                    ...
                    foreach (var item in queryitems)
                    {
                        ...
                        queryparameters.Add(item.Key.Trim(), value);
                    }
                    int connectorId = -1;
                    if (queryparameters.ContainsKey("idTag") && queryparameters.ContainsKey("connectorId"))
                    {

                        try
                        {
                            ChargePointStatus status = null;
                            connectorId = int.Parse(queryparameters["connectorId"]);
                            if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                            {
                                // Send message to chargepoint
                                if (status.Protocol == Protocol_OCPP20)
                                ...
                                {
                                    // OCPP V1.6
                                    await RemoteStartTransaction16(status, context, connectorId, queryparameters["idTag"]);
                                }
                            }
                            else
                            {
                        ...
                        {
                            _logger.LogError(exp, "OCPP RemoteStartTransaction failed.", exp.Message);
                            context.Response.StatusCode = (int)HttpStatusCode.InternalServerError;
                        }
                    }
                }
                    ...
                    if (!string.IsNullOrEmpty(urlChargePointId))
                    {
                        try
                        {
                            ...
                            if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                            {
                                ...
                                {
                                    // OCPP V2.0
                                    // await RemoteStaTransaction16(status, context);
                                }
                                else
                                {
                                    // OCPP V1.6
                    ...
                    if (!string.IsNullOrEmpty(urlChargePointId))
                    {
                        try
                        {
                            ...
                            if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                            {
                                ...
                                {
                                    // OCPP V1.6
                                    await GetConfiguration(status, context);
                                }
                            }
                            else
                            {
                ...
                else if (cmd == "SetConfig")
                {
                    ...
                    if (!string.IsNullOrEmpty(urlChargePointId) && !string.IsNullOrEmpty(bodyString))
                    {
                        string[] parametersArray = bodyString.Split("&");
                        ...
                        try
                        {
                            ...
                            if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                            {
                                ...
                                {
                                    // OCPP V1.6
                                    await SetConfiguration(status, context, parametersArray);
                                }
                            }
                            else
                            {
                ...
                else if (cmd == "SetChargingProfile")
                {
                    string bodyString;
                    try
                    {
                    ...
                    if (!string.IsNullOrEmpty(urlChargePointId) && !string.IsNullOrEmpty(bodyString))
                    {
                        ...
                        try
                        {
                            ...
                            if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                            {
                                ...
                                {
                                    // OCPP V1.6
                                    await SetChargingProfile(status, context, parametersArray);
                                }
                            }
                            else
                            {
                ...
                else if (cmd == "GetCompositeSchedule")
                {
                    ...
                    if (!string.IsNullOrEmpty(urlChargePointId))
                    {
                        try
                        {
                            ...
                            if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                            {
                                ...
                                {
                                    // OCPP V1.6
                                    await GetCompositeSchedule(status, context);
                                }
                            }
                            else
                            {
